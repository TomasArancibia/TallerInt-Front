...
export default function UbicacionesInstituciones(){
  ...
  return (
    <div className="flex flex-col gap-4">
      <header className="rounded-2xl bg-white px-6 py-4 text-slate-900 shadow-sm ring-1 ring-slate-200"><h1 className="text-lg font-semibold">Instituciones</h1></header>
      ...
      {status==='ok' && (
        <section>
          <div className="mb-2 text-sm text-slate-600">Resultados: {hospitales.length}</div>
          <div className={tableWrapper}>
            <table className={tableClass}><thead><tr><th className={headerCell}>ID</th><th className={headerCell}>Nombre</th></tr></thead>
              <tbody>
                {hospitales.map(h=> (<tr key={h.id_hospital}><td className={dataCell}>{h.id_hospital}</td><td className={dataCell}>{h.nombre}</td></tr>))}
              </tbody>
            </table>
          </div>
        </section>
