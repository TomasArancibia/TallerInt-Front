...
export default function UbicacionesHabitaciones(){
  ...
  const filtradas = useMemo(()=>{
    return habitaciones.filter(h=>{
      ...
      if(instSel.length){ const p=pisoById[h.id_piso]; const e=p?edifById[p.id_edificio]:null; if(!e || !instSel.includes(String(e.id_hospital))) return false; }
      if(servSel.length && !servSel.includes(String(h.id_servicio))) return false;
      return true;
    });
  },[habitaciones,pisoSel,edifSel,instSel,servSel,pisoById,edifById]);
  ...
  return (
    <div className="flex flex-col gap-4">
      <header className="rounded-2xl bg-white px-6 py-4 text-slate-900 shadow-sm ring-1 ring-slate-200"><h1 className="text-lg font-semibold">Habitaciones</h1></header>
