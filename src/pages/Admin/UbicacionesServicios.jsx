...
export default function UbicacionesServicios() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error("No se pudieron cargar datos");
  ...
  const pageItems = useMemo(
    () => servicios.slice(startIdx, endIdx),
    [servicios, startIdx, endIdx]
  );
        ...
        <section>
          ...
          <div className="mt-3 flex items-center justify-between text-sm">
            <div className="text-slate-600">
              Mostrando {servicios.length === 0 ? 0 : startIdx + 1}-{endIdx} de {" "}
              {servicios.length}
            </div>
